...
$stmt_next_num = $pdo->prepare("SELECT MAX(CAST(agenda_urut AS UNSIGNED)) FROM surat_masuk WHERE YEAR(tanggal_diterima) = ?");
...
<div class="pb-10 md:pb-6">
    <div id="form-masuk-container" class="bg-gradient-to-br from-white to-blue-50 rounded-2xl shadow-xl p-4 md:p-6 animate-fade-in border border-blue-100 mx-2 md:mx-0 mt-4 md:mt-0">
        <div class="flex justify-between items-center mb-4 md:mb-6 border-b border-blue-200 pb-2 md:pb-3">
            <h3 class="text-xl md:text-2xl font-bold text-gray-800 flex items-center">
                ...
                <i class="fas fa-envelope ml-2 md:ml-3 text-primary"></i>
            </h3>
            <button id="toggle-form-masuk-btn" class="text-primary hover:text-secondary text-lg md:text-xl p-1 md:p-2">
                <i class="fas fa-chevron-up"></i>
            </button>
        </div>
        
        <form id="form-masuk-body" method="POST" action="/surat-masuk" enctype="multipart/form-data" class="space-y-4 md:space-y-6 transition-all duration-500">
                ...
                <div class="space-y-4 md:space-y-5">
                    <div>
                        ...
                        <div class="flex flex-col md:flex-row md:items-center space-y-2 md:space-y-0 md:space-x-2">
                            <input type="text" id="agenda_klasifikasi" name="agenda_klasifikasi" list="klasifikasi-list" class="flex-1 px-3 md:px-4 py-2 md:py-3 rounded-xl border border-gray-300" placeholder="Ketik/Pilih Klasifikasi..." required />
                            ...
                            <input type="text" id="agenda_urut" name="agenda_urut" value="<?php echo $next_nomor_urut; ?>" class="w-full md:w-24 px-3 md:px-4 py-2 md:py-3 rounded-xl border border-gray-300 text-center" placeholder="No. Urut">
                            <span class="hidden md:block text-gray-500 pt-2">/</span>
                            <select id="tahun_penomoran_masuk" name="tahun_penomoran" class="w-full md:w-28 px-3 md:px-4 py-2 md:py-3 rounded-xl border border-gray-300 bg-white" required>
                            ...
                            <button type="button" id="checkAgendaBtn" class="px-3 md:px-4 py-2 md:py-3 bg-indigo-100 text-indigo-600 rounded-xl hover:bg-indigo-200 mt-2 md:mt-0" title="Cek ketersediaan No. Urut Agenda">
            ...
            <div class="mt-6 flex flex-col-reverse md:flex-row justify-end space-y-reverse space-y-4 md:space-y-0 md:space-x-4">
                ...
                <button type="submit" name="simpan_surat_masuk" class="px-4 md:px-6 py-2 md:py-3 bg-gradient-to-r from-primary to-secondary text-white rounded-xl shadow-md hover:shadow-lg">
...
<div id="list-masuk-container" class="mt-6 md:mt-8 bg-gradient-to-br from-white to-blue-50 rounded-2xl shadow-xl p-4 md:p-6 animate-fade-in border border-blue-100 mx-2 md:mx-0">
    <div class="flex flex-col gap-4 mb-4 md:mb-6">
        ...
        <form id="searchFormMasuk" class="w-full flex flex-col md:flex-row items-stretch md:items-center gap-3">
            <!-- Filter Tahun -->
            <select id="filterTahunMasuk" name="filter_tahun" class="w-full md:w-44 px-3 md:px-4 py-2 md:py-3 rounded-xl border border-gray-300 bg-white focus:ring-2 focus:ring-indigo-300 focus:border-indigo-500 transition duration-200">
            ...
            <div class="relative w-full">
                ...
                <input type="text" id="searchInputMasuk" name="search" class="w-full pl-10 pr-4 py-2 md:py-3 border border-gray-300 rounded-xl" placeholder="Cari perihal, asal surat...">
            </div>
        </form>
    </div>
    ...
    <div id="paginationContainerMasuk" class="mt-4 md:mt-6">
        <?php
        if ($total_pages > 1) {
            ...
            echo '<div><button onclick="document.getElementById(\'searchFormMasuk\').fetchData(2)" class="px-4 py-2 rounded-lg border border-gray-300 bg-white text-primary hover:bg-gray-50 w-full md:w-auto">Selanjutnya <i class="fas fa-arrow-right ml-1"></i></button></div>';
            echo '</div>';
        }
        ?>
    </div>
