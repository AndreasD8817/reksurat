...
<div class="bg-white rounded-2xl shadow-lg p-6 animate-fade-in">
    <h3 class="text-xl font-semibold text-gray-800 mb-6 border-b pb-3">Form Penomoran Surat Keluar</h3>
    <form method="POST" action="/surat-keluar" enctype="multipart/form-data">
            ...
            <div class="space-y-5">
                <div>
                    ...
                    <div class="flex items-center space-x-2">
                        <input type="text" name="kode_klasifikasi" class="flex-1 px-4 py-3 rounded-xl border" placeholder="Klasifikasi" required />
                        <span class="text-gray-500 pt-2">/</span>
                        <input type="number" id="nomor_urut_keluar" name="nomor_urut" class="w-full px-4 py-3 rounded-xl border text-center" placeholder="No. Urut" required />
                ...
                <div><label class="block text-sm font-medium text-gray-700 mb-2">Tujuan</label><input type="text" name="tujuan" class="w-full px-4 py-3 rounded-xl border" required /></div>
                <div><label class="block text-sm font-medium text-gray-700 mb-2">Konseptor</label><input type="text" name="konseptor" class="w-full px-4 py-3 rounded-xl border" /></div>
            </div>
            <div class="space-y-5">
                ...
                <div><label class="block text-sm font-medium text-gray-700 mb-2">Hubungan Dgn Surat No</label><input type="text" name="hub_surat_no" class="w-full px-4 py-3 rounded-xl border" /></div>
                <div><label class="block text-sm font-medium text-gray-700 mb-2">Keterangan</label><textarea name="keterangan" class="w-full px-4 py-3 rounded-xl border h-32"></textarea></div>
            </div>
                ...
                <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md">
                    <div class="space-y-1 text-center">
                        ...
                        <div class="flex text-sm text-gray-600">
                            <label for="file-upload-keluar" class="relative cursor-pointer bg-white rounded-md font-medium text-primary hover:text-secondary focus-within:outline-none">
                                <span>Unggah file</span>
                                <input id="file-upload-keluar" name="file_lampiran" type="file" class="sr-only">
                            </label>
        ...
        <div class="mt-8 flex justify-end space-x-4">
            <button type="reset" class="px-6 py-3 border rounded-xl">Reset</button>
            <button type="submit" name="simpan_surat" class="px-6 py-3 bg-primary text-white rounded-xl shadow-md">Simpan</button>
        </div>
    </form>
</div>
...
<div class="mt-8 bg-white rounded-2xl shadow-lg p-6 animate-fade-in">
    ...
    <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
        <h3 class="text-lg font-semibold text-gray-800 flex items-center"><i class="fas fa-list-alt text-primary mr-2"></i> Daftar Surat Keluar</h3>
        <form id="searchFormKeluar" class="w-full md:w-96">
            <div class="relative">
                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-gray-400"><i class="fas fa-search"></i></div>
                <input type="text" id="searchInputKeluar" name="search" class="w-full pl-10 pr-4 py-3 border rounded-xl" placeholder="Ketik untuk mencari surat...">
            </div>
        </form>
    </div>
    <div class="overflow-x-auto rounded-xl border">
        <table class="min-w-full divide-y">
            <thead class="bg-gray-50">
                <tr>
                    <th class="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase">No. Surat</th>
                    ...
                    <th class="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase">Tujuan</th>
                    <th class="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase">Lampiran</th>
                </tr>
            </thead>
            <tbody id="tableBodyKeluar" class="bg-white divide-y">
                <?php foreach ($surat_keluar_list as $surat): ?>
                     <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4"><?php echo htmlspecialchars($surat['tgl_formatted']); ?></td>
                        <td class="px-6 py-4"><?php echo htmlspecialchars($surat['perihal']); ?></td>
                        <td class="px-6 py-4"><?php echo htmlspecialchars($surat['tujuan']); ?></td>
                        <td class="px-6 py-4">
    ...
    <div id="paginationContainerKeluar" class="mt-6">
         ...
         if ($total_pages > 1) {
             ...
             echo '<div>Halaman 1 dari ' . $total_pages . '</div>';
             echo '<div><button onclick="fetchData(2)" class="px-4 py-2 rounded-lg border">Selanjutnya</button></div>';
             echo '</div>';
         }
         ?>
    </div>
