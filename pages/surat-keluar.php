...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['simpan_surat'])) {
    ...
    if ($is_exists) {
        ...
        if (!isset($_SESSION['error_message'])) {
            ...
            log_activity($pdo, "Menambah Surat Keluar '{$nomor_lengkap}'", ['sesudah' => $data_baru]);
...
$limit = 10;
$stmt_data = $pdo->prepare("SELECT *, DATE_FORMAT(tanggal_surat, '%d-%m-%Y') as tgl_formatted FROM surat_keluar ORDER BY tanggal_surat DESC, id DESC LIMIT ?");
...
$stmt_years = $pdo->query("SELECT DISTINCT YEAR(tanggal_surat) as year FROM surat_keluar WHERE YEAR(tanggal_surat) IS NOT NULL ORDER BY year ASC");
        ...
        <div class="overflow-x-auto rounded-xl border border-gray-200 shadow-sm">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gradient-to-r from-primary to-secondary">
                    <tr>
                        <th class="px-4 md:px-6 py-2 md:py-4 text-left text-xs font-semibold text-white uppercase tracking-wider sortable-col cursor-pointer" data-sort-col="nomor_surat_lengkap" data-sort-order="asc">
                            No. Surat <span class="sort-icon"></span>
                        </th>
                        <th class="px-4 md:px-6 py-2 md:py-4 text-left text-xs font-semibold text-white uppercase tracking-wider hidden md:table-cell sortable-col cursor-pointer" data-sort-col="tanggal_surat" data-sort-order="desc">
                            Tanggal <span class="sort-icon"><i class="fas fa-sort-down"></i></span>
                        </th>
                        ...
                        <th class="px-4 md:px-6 py-2 md:py-4 text-left text-xs font-semibold text-white uppercase tracking-wider hidden md:table-cell sortable-col cursor-pointer" data-sort-col="tujuan" data-sort-order="asc">
                            Tujuan <span class="sort-icon"></span>
                        </th>
