...
// Fungsi untuk menangani unggahan file disposisi
function handleDisposisiFileUpload($fileInputName) {
    if (isset($_FILES[$fileInputName]) && $_FILES[$fileInputName]['error'] === UPLOAD_ERR_OK) {
        $file = $_FILES[$fileInputName];
        $fileName = time() . '_' . basename($file['name']);
        
        // Simpan file ke subfolder khusus untuk disposisi agar rapi
...
// Logika untuk menyimpan data disposisi baru saat form disubmit
...
// Ambil daftar surat masuk yang BELUM ada di tabel disposisi
$surat_untuk_disposisi = $pdo->query(
    "SELECT sm.id, sm.nomor_agenda_lengkap, sm.perihal 
     FROM surat_masuk sm
     WHERE NOT EXISTS (SELECT 1 FROM disposisi_sekwan ds WHERE ds.surat_masuk_id = sm.id)
     ORDER BY sm.id DESC"
)->fetchAll(PDO::FETCH_ASSOC);

// Ambil data disposisi yang sudah ada untuk ditampilkan di tabel (halaman pertama)
$limit = 10;
...
$pageTitle = 'Disposisi Surat Masuk (Sekwan)';
...
<div class="bg-gradient-to-br from-white to-blue-50 rounded-2xl shadow-xl p-6 animate-fade-in border border-blue-100">
    <h3 class="text-2xl font-bold text-gray-800 mb-6 border-b border-blue-200 pb-3 flex items-center">
        <span class="bg-gradient-to-r from-primary to-secondary text-transparent bg-clip-text">Form Disposisi Surat</span>
        <i class="fas fa-share-square ml-3 text-primary"></i>
    </h3>
    
    <form method="POST" action="/disposisi-sekwan" class="space-y-6" enctype="multipart/form-data">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <label for="surat_masuk_id" class="block text-sm font-medium text-gray-700 mb-2">Nomor Agenda Surat</label>
                <select id="surat_masuk_id" name="surat_masuk_id" class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-indigo-500 transition" required>
                    ...
                    <?php foreach ($surat_untuk_disposisi as $surat): ?>
                        <option value="<?php echo $surat['id']; ?>">
                            <?php echo htmlspecialchars($surat['nomor_agenda_lengkap']) . ' - ' . htmlspecialchars(substr($surat['perihal'], 0, 50)) . '...'; ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div>
                <label for="nama_pegawai" class="block text-sm font-medium text-gray-700 mb-2">Diteruskan kepada (Pegawai)</label>
                <input type="text" id="nama_pegawai" name="nama_pegawai" class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-indigo-500 transition" placeholder="Contoh: Budi, Staf Umum" required>
            </div>
            <div class="md:col-span-2">
                <label for="catatan_disposisi" class="block text-sm font-medium text-gray-700 mb-2">Isi / Catatan Disposisi (Opsional)</label>
                <textarea id="catatan_disposisi" name="catatan_disposisi" rows="4" class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-indigo-500 transition" placeholder="Contoh: Mohon segera ditindaklanjuti dan siapkan balasan."></textarea>
            </div>
            <div class="md:col-span-2">
                ...
                <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-dashed border-gray-300 rounded-xl hover:border-primary cursor-pointer relative group">
                    ...
                    <div class="space-y-1 text-center">
                         <i class="fas fa-cloud-upload-alt text-4xl text-gray-400 group-hover:text-primary transition-colors"></i>
...
<div class="mt-8 bg-white rounded-2xl shadow-xl p-6">
    <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
        <h3 class="text-xl font-bold text-gray-800">Daftar Disposisi Tersimpan</h3>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody id="tableBodyDisposisi" class="bg-white divide-y divide-gray-200">
                <?php if (empty($disposisi_list)): ?>
                    <tr><td colspan="6" class="text-center p-8 text-gray-500">Belum ada data disposisi.</td></tr>
                <?php else: ?>
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <?php if (isset($_SESSION['user_role']) && $_SESSION['user_role'] === 'admin'): ?>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <button onclick="confirmDelete('disposisi-sekwan', <?php echo $disposisi['id']; ?>)" class="text-red-600 hover:text-red-900">Batalkan</button>
                                </td>
                            <?php endif; ?>
                        </tr>
                    <?php endforeach; ?>
